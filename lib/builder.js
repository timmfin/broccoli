...
Builder.prototype.build = function (willReadStringTree) {
  ...
  function readAndReturnNodeFor (tree) {
    ...
    if (typeof tree === 'string') {
      ...
      throw new Error('Invalid tree found. You must supply a path or an object with a `read` (deprecated) or `rebuild` function.');
    } else {
  ...
  if (typeof tree.rebuild === 'function' &&
    ...
    if (typeof tree.read === 'function' &&
      ...
      var description = tree.description ||
        (tree.constructor && tree.constructor.name) ||
        ('' + tree)
      console.warn('[API Update] Warning: .read trees are deprecated: ' + description)
